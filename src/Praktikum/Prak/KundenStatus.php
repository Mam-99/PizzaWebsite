...
class KundenStatus extends Page
{
    ...
    protected function getViewData():array
    {
        ...
        $array = array();
        $guest = (isset($_SESSION["guestID"]))? $_SESSION["guestID"] : 17;

        $query = "select article.name, article.picture, article.price, ordered_article_id, ordering_id, status, address
                 from article inner join ordered_article using(article_id) inner join ordering using(ordering_id)
                 where status > 2 and ordering_id = {$guest} order by ordered_article_id";
